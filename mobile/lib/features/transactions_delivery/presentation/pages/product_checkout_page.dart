...
class CheckoutItem {
  ...
  const CheckoutItem({required this.productId, required this.name, required this.unitPrice, required this.quantity, required this.sellerDeliveryAvailable});
}
...
class _ProductCheckoutPageState extends State<ProductCheckoutPage> {
  ...
  bool get _sellerDeliveryAvailable => widget.items.isNotEmpty && widget.items.every((x) => x.sellerDeliveryAvailable);
  double get _total => widget.items.fold(0, (total, item) => total + item.unitPrice * item.quantity);
  ...
  Future<void> _submit() async {
    ...
    try {
      final order = await _api.createProductOrder(
        items: widget.items.map((x) => {'productId': x.productId, 'quantity': x.quantity}).toList(),
        method: _method,
        location: _location,
      );
      if (!mounted) return;
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => TransactionDetailPage(id: order.id, order: true)));
    } catch (error) {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            DeliveryMethodSelector(value: _method, sellerDeliveryAvailable: _sellerDeliveryAvailable, onChanged: (value) => setState(() => _method = value)),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Total', style: Theme.of(context).textTheme.titleLarge), Text('\$${_total.toStringAsFixed(2)}', style: Theme.of(context).textTheme.titleLarge)]),
            const SizedBox(height: 20),
            ElevatedButton(onPressed: _submitting || widget.items.isEmpty ? null : _submit, child: Text(_submitting ? 'Placing Order...' : 'Place Order')),
          ],
        ),
      );
}
